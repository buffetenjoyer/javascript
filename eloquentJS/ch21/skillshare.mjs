...
import {Router} from "./router.mjs";
 ...
 response, next) {
    ...
    if (!resolved) return next();                          // error is not a falsy value, but undefined is, and variable resolved can = undefined
                                                           // if router.resolve() returns undefined, which it can when there's no existing handler
                                                           // in the router.routes array that can handle the request you're dealing with.
                                                           // (check the resolve() method in file router.mjs, it's possible for it to return 
                                                           // nothing (which means it returns undefined))
